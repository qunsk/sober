...
<c:choose>
	<c:when test="${! empty sessionScope.nickname }">
		...
		<c:if test="${empty heart_sel}">
			<a href="javascript:;" idx="${food.f_num}" name="${food.nickname}" class="heart-click heart_icon${food.f_num}" > 
		...
		<c:if test="${!empty heart_sel}">
			...
			<c:forEach var="heart" items="${heart_sel }">
				<c:if test="${heart.f_num == food.f_num}">
...
<br><span id="heart${food.f_num}">${food.like_count}</span>
</body>
</html>